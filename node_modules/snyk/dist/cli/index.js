...
async function runCommand(args) {
    ...
    if (args.command === 'test') {
        ...
        if (jsonOutputFile) {
            const jsonOutputFileStr = jsonOutputFile;
            const fullOutputFilePath = getFullPath(jsonOutputFileStr);
            saveJsonResultsToFile(strip_ansi_1.default(commandResult.getJsonResult()), fullOutputFilePath);
        }
    }
    return res;
}
async function handleError(args, error) {
    ...
    if (jsonOutputFile && error.jsonStringifiedResults) {
        ...
        saveJsonResultsToFile(strip_ansi_1.default(error.jsonStringifiedResults), fullOutputFilePath);
    }
...
async function main() {
    ...
    try {
        ...
        const jsonFileOptionSet = 'json-file-output' in args.options;
        if (jsonFileOptionSet) {
            ...
            if (!jsonFileOutputValue || typeof jsonFileOutputValue !== 'string') {
                throw new json_file_output_bad_input_error_1.JsonFileOutputBadInputError();
            }
